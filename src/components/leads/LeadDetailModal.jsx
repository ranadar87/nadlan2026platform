                ...
                <div className="absolute left-0 top-full mt-1 bg-card border border-border rounded-xl shadow-card-hover z-50 overflow-hidden min-w-36">
                  {STATUS_OPTIONS.map(opt => (
                    <button key={opt.value} onClick={() => { onStatusChange(lead, opt.value); setShowStatusMenu(false); }}
                      ...
                      <span className={`w-2 h-2 rounded-full ${opt.color.includes("primary") ? "bg-primary" : opt.color.includes("info") ? "bg-info" : opt.color.includes("success") ? "bg-success" : opt.color.includes("destructive") ? "bg-destructive" : opt.color.includes("accent") ? "bg-accent" : "bg-muted-foreground"}`} />
                      {opt.label}
                    </button>
                  ))}
                </div>
